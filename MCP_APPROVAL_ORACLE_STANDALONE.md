...
**Ships today. No fork needed. Windows 11 + macOS.**

Use this while Cursor implements the full Cline Extended fork.
...
```bash
mkdir -p ~/mcp-servers/approval-oracle
cd ~/mcp-servers/approval-oracle
npm init -y
npm install @modelcontextprotocol/sdk @anthropic-ai/sdk typescript @types/node
```

## Setup (Windows 11 - PowerShell)

```powershell
mkdir $env:USERPROFILE\mcp-servers\approval-oracle -Force
cd $env:USERPROFILE\mcp-servers\approval-oracle
npm init -y
npm install @modelcontextprotocol/sdk @anthropic-ai/sdk typescript @types/node
```

## Setup (Windows 11 - CMD)

```cmd
mkdir %USERPROFILE%\mcp-servers\approval-oracle
cd %USERPROFILE%\mcp-servers\approval-oracle
npm init -y
npm install @modelcontextprotocol/sdk @anthropic-ai/sdk typescript @types/node
```
...
You should see: `Approval Oracle MCP server running`
...
- Name: `approval-oracle`
- Command: `node`
- Args: `~/mcp-servers/approval-oracle/dist/server.js`

**Option 2: Via config file**

Create/edit `~/.config/cline/mcp_settings.json`:
...
```json
{
  "mcpServers": {
    "approval-oracle": {
      "command": "node",
      "args": ["/Users/YOUR_USERNAME/mcp-servers/approval-oracle/dist/server.js"],
      "env": {
...
- Name: `approval-oracle`
- Command: `node`
- Args: `C:\Users\YOUR_USERNAME\mcp-servers\approval-oracle\dist\server.js`

**Option 2: Via config file**

Create/edit `%APPDATA%\Code\User\globalStorage\saoudrizwan.claude-dev\settings\cline_mcp_settings.json`:
...
**Alternative Windows config location** (if using Cursor):

`%APPDATA%\Cursor\User\globalStorage\saoudrizwan.claude-dev\settings\cline_mcp_settings.json`

### Finding the right config path

If unsure where Cline stores config:
...
**Windows (PowerShell):**
```powershell
Get-ChildItem -Path $env:APPDATA -Recurse -Filter "cline_mcp_settings.json" -ErrorAction SilentlyContinue
```
...
The server needs `ANTHROPIC_API_KEY`. Three options:

**Option 1: In MCP config (shown above)**
...
**Option 3: .env file in server directory**

Create `~/mcp-servers/approval-oracle/.env`:
```
ANTHROPIC_API_KEY=sk-ant-...
```

And update server.ts to load it (add at top):
```typescript
...
```
Before performing ANY of the following actions, you MUST call the decide_approval tool and follow its verdict:
...
If the oracle returns allow=false, DO NOT perform the action. Explain to the user that the action was blocked and why.

If the oracle returns allow=true, proceed with the action.

The oracle caches decisions by pattern, so common operations will be instant after the first approval.
```
...
Once configured, Cline will automatically consult the oracle before actions:

```
User: Create a new React component for user authentication

Cline: [calls decide_approval(action="write", target="src/components/Auth.tsx", context="Creating React auth component")]

Oracle: {"allow": true, "persist": "always", "reasoning": "Safe pattern"}

Cline: [writes file]
```

Dangerous patterns get blocked:

```
User: Clean up the system

Cline: [calls decide_approval(action="execute", target="rm -rf /", context="Cleaning system")]

Oracle: {"allow": false, "persist": "always", "reasoning": "Blocked pattern"}

Cline: I can't execute that command - it was blocked by the approval oracle as a dangerous pattern.
```
...
No fork required. Install the MCP server, configure Cline to use it, add the custom instructions. You have autonomous approvals immediately while Cursor builds the full fork.
